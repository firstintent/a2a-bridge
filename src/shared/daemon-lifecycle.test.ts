...
describe("DaemonLifecycle", () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), "a2a-bridge-lifecycle-test-"));
  ...
  test("kill refuses to signal a live process that is not an A2aBridge daemon", async () => {
    const lc = createLifecycle();
    ...
    const result = await lc.kill();
    expect(result).toBe(false);
    expect(logs.some((l) => l.includes("NOT an A2aBridge daemon"))).toBe(true);
