...
export async function runCodex(args: string[]) {
  ...
  checkOwnedFlagConflicts(args, "a2a-bridge codex", OWNED_FLAGS);

  // Specifically check for --enable tui_app_server (not all --enable values)
  for (let i = 0; i < args.length; i++) {
    if (args[i] === "--enable" && args[i + 1] === "tui_app_server") {
      console.error(`Error: "--enable tui_app_server" is automatically set by a2a-bridge codex.`);
      console.error("");
  ...
  console.error("[a2a-bridge] Ensuring daemon is running...");
  try {
    lifecycle.clearKilled();
    await lifecycle.ensureRunning();
    console.error("[a2a-bridge] Daemon is ready.");
  } catch (err: any) {
    console.error(`[a2a-bridge] Failed to start daemon: ${err.message}`);
    console.error("[a2a-bridge] Try: a2a-bridge kill && a2a-bridge claude");
    process.exit(1);
  }
  ...
  if (status?.proxyUrl) {
    ...
    console.error(`[a2a-bridge] No daemon status found, using config default: ${proxyUrl}`);
  }

  try {
    await waitForProxyReady(proxyUrl);
  } catch (err: any) {
    console.error(`[a2a-bridge] ${err.message}`);
    process.exit(1);
  }

  // Save terminal state and launch Codex with protection
  console.log(`Connecting Codex TUI to A2aBridge at ${proxyUrl}...`);
