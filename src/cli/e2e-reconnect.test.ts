...
const TEST_STATE_DIR = `/tmp/a2a-bridge-e2e-test-${TEST_CONTROL_PORT}`;
...
function launchDaemon(): ChildProcess {
  ...
  const proc = spawn(process.execPath, ["run", DAEMON_PATH], {
    env: {
      ...process.env,
      A2A_BRIDGE_CONTROL_PORT: String(TEST_CONTROL_PORT),
      A2A_BRIDGE_STATE_DIR: TEST_STATE_DIR,
      CODEX_WS_PORT: String(TEST_APP_PORT),
      CODEX_PROXY_PORT: String(TEST_PROXY_PORT),
      A2A_BRIDGE_IDLE_SHUTDOWN_MS: "60000", // don't auto-shutdown during tests
    },
    stdio: "pipe",
  });
  return proc;
